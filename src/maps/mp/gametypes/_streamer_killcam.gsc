...
init()
{
	logprint("_streamer_killcam::init\n");
	...
	if(game["firstInit"])
	{
		maps\mp\gametypes\global\_global::precacheString2("STRING_STREAMERSYSTEM_KILLCAM_PROPOSAL_KEY_MULTIKILL", &"Press ^3[F]^7 to replay ^2multi-kill ^7 of:");
		maps\mp\gametypes\global\_global::precacheString2("STRING_STREAMERSYSTEM_KILLCAM_PROPOSAL_KEY_MULTIKILL_2", &"Press ^3[F]^7 to replay ^22 kills ^7 of:");
		maps\mp\gametypes\global\_global::precacheString2("STRING_STREAMERSYSTEM_KILLCAM_PROPOSAL_KEY_MULTIKILL_3", &"Press ^3[F]^7 to replay ^23 kills ^7 of:");
		maps\mp\gametypes\global\_global::precacheString2("STRING_STREAMERSYSTEM_KILLCAM_PROPOSAL_KEY_MULTIKILL_4", &"Press ^3[F]^7 to replay ^24 kills ^7 of:");
		maps\mp\gametypes\global\_global::precacheString2("STRING_STREAMERSYSTEM_KILLCAM_PROPOSAL_KEY_MULTIKILL_5", &"Press ^3[F]^7 to replay ^25 kills ^7 of:");
		maps\mp\gametypes\global\_global::precacheString2("STRING_STREAMERSYSTEM_KILLCAM_PROPOSAL_KEY_NOZOOM", &"Press ^3[F]^7 to replay ^2nozoom ^7 of:");
		maps\mp\gametypes\global\_global::precacheString2("STRING_STREAMERSYSTEM_KILLCAM_PROPOSAL_KEY_BASH", &"Press ^3[F]^7 to replay ^2bash ^7 of:");

		maps\mp\gametypes\global\_global::precacheString2("STRING_STREAMERSYSTEM_KILLCAM_REPLAY", &"Streamer killcam replay");
	}
...
onMenuResponse(menu, response)
{
	...
	//{
		if (response == "streamersystem_killcam_open")
		{
			self HUD_SetRowsInMenu();

			return true;
		}
		else if (response == "streamersystem_killcam_close")
		{
			return true;
		}
		else
		{
			i = -1;
	    		switch(response)
			{
				case "streamersystem_killcam_1": i = 0; break;
				case "streamersystem_killcam_2": i = 1; break;
				case "streamersystem_killcam_3": i = 2; break;
				case "streamersystem_killcam_4": i = 3; break;
				case "streamersystem_killcam_5": i = 4; break;
				case "streamersystem_killcam_6": i = 5; break;
				case "streamersystem_killcam_7": i = 6; break;
				case "streamersystem_killcam_8": i = 7; break;
				case "streamersystem_killcam_9": i = 8; break;
				case "streamersystem_killcam_0": i = 9; break;
			}
...
showKillcamProposingForPlayer(killId)
{
	...
	if(!isdefined(self.autoSpectating_KillcamProposingBG)) {
		//self.autoSpectating_KillcamProposingBG = maps\mp\gametypes\global\_global::addHUDClient(self, -80, 410, undefined, undefined, undefined, undefined, "center", "top"); // addHUDClient(player, x, y, fontSize, color, alignX, alignY, horzAlign, vertAlign)
		self.autoSpectating_KillcamProposingBG = maps\mp\gametypes\global\_global::addHUDClient(self, 240, 410, undefined, undefined, undefined, undefined, "center", "top"); // addHUDClient(player, x, y, fontSize, color, alignX, alignY, horzAlign, vertAlign)
	}
	...
	if(!isdefined(self.autoSpectating_KillcamProposingBGline)) {
		//self.autoSpectating_KillcamProposingBGline = maps\mp\gametypes\global\_global::addHUDClient(self, -80, 410+22, undefined, undefined, undefined, undefined, "center", "top"); // addHUDClient(player, x, y, fontSize, color, alignX, alignY, horzAlign, vertAlign)
		self.autoSpectating_KillcamProposingBGline = maps\mp\gametypes\global\_global::addHUDClient(self, 240, 410+22, undefined, undefined, undefined, undefined, "center", "top"); // addHUDClient(player, x, y, fontSize, color, alignX, alignY, horzAlign, vertAlign)
	}
	...
	if(!isdefined(self.autoSpectating_KillcamProposingText)) {
		//self.autoSpectating_KillcamProposingText = maps\mp\gametypes\global\_global::addHUDClient(self, 0, 410+2, 0.7, (1,1,1), "center", "top", "center", "top");
		self.autoSpectating_KillcamProposingText = maps\mp\gametypes\global\_global::addHUDClient(self, 320, 410+2, 0.7, (1,1,1), "center", "top", "center", "top");
	}
	...
	if(!isdefined(self.autoSpectating_KillcamProposingPlayer)) {
		//self.autoSpectating_KillcamProposingPlayer = maps\mp\gametypes\global\_global::addHUDClient(self, 0, 410+12, 0.7, (1,1,1), "center", "top", "center", "top");
		self.autoSpectating_KillcamProposingPlayer = maps\mp\gametypes\global\_global::addHUDClient(self, 320, 410+12, 0.7, (1,1,1), "center", "top", "center", "top");
	}
...
hideKillcamProposingForPlayer()
{
	if(isdefined(self.autoSpectating_KillcamProposingBG))
	{
		self.autoSpectating_KillcamProposingBG destroy();
		self.autoSpectating_KillcamProposingBG = undefined;
	}
	if(isdefined(self.autoSpectating_KillcamProposingBGline))
	{
		self.autoSpectating_KillcamProposingBGline destroy();
		self.autoSpectating_KillcamProposingBGline = undefined;
	}
	if(isdefined(self.autoSpectating_KillcamProposingText))
	{
		self.autoSpectating_KillcamProposingText destroy();
		self.autoSpectating_KillcamProposingText = undefined;
	}
	if(isdefined(self.autoSpectating_KillcamProposingPlayer))
	{
		self.autoSpectating_KillcamProposingPlayer destroy();
...
watchIntenseSituation()
{
	...
	for (;;)
	{
		...
		if (teamPlayersCount > 0 && ((alliesAlive / (teamPlayersCount / 2.0)) <= 0.5 || (axisAlive / (teamPlayersCount / 2.0)) <= 0.5))
...
HUD_SetRowsInMenu()
{
	...
	if (str == "")
		self maps\mp\gametypes\global\_global::setClientCvar2("ui_spectatorsystem_menu_text", "No record to play!");
	else
		self maps\mp\gametypes\global\_global::setClientCvar2("ui_spectatorsystem_menu_text", str);
}
...
getTimeString(time, bombPlanted)
{
	if (level.gametype == "sd")
	{
		...
		return maps\mp\gametypes\global\_global::formatTime((int)((level.timelimit * 60) - ((int)((time - level.starttime)/1000)) - level.timeout_elapsedTime));
}
