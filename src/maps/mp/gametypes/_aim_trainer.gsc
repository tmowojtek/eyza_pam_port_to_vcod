...
*/
	...
	{
		logprint("aim_trainer: invalid size(0) of 'training_target' or 'trainer_trigger'\n");
		return;
	}
	if (targets.size != targets_damage.size)
	{
		logprint("aim_trainer: invalid number of 'trainer_target' vs 'trainer_trigger'\n");
	...
	{
		logprint("aim_trainer: invalid number of 'activators' vs 'targets_damage' vs 'activators_hint'\n");
...
player_loop()
{
	...
	while(1)
	{
		...
		if (firstRun)
		{
			...
			self setWeaponSlotClipAmmo("weapon2", 999);
			firstRun = false;
		}
		...
		if (current_clipAmmo == last_clipAmmo && current_clipAmmo != new_clipAmmo)
		{
			...
			self setweaponslotclipammo(current, new_clipAmmo);
