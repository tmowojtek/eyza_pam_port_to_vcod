...
init()
{
	logprint("_weapon_drop::init\n");
	
	maps\mp\gametypes\global\_global::addEventListener("onCvarChanged", ::onCvarChanged);

	// Player's weapon drop
	maps\mp\gametypes\global\_global::registerCvar("scr_allow_primary_drop", "BOOL", 0);
	maps\mp\gametypes\global\_global::registerCvar("scr_allow_secondary_drop", "BOOL", 0);
...
dropped_weapon_think(weaponname)
{
	...
	for (;;)
	{
		...
		if (isDefined(dropedWeaponEntitiy))
		{
			...
			droppedWeaponName = maps\mp\gametypes\global\_global::getsubstr(dropedWeaponEntitiy.classname, "weapon_".size);
