...
init()
{
	...
	maps\mp\gametypes\global\_global::addEventListener("onSpawnedStreamer",  ::onSpawnedStreamer);
	maps\mp\gametypes\global\_global::addEventListener("onConnectedAll",    	::onConnectedAll);
...
exit()
{
	...
	self maps\mp\gametypes\global\_global::setClientCvar2("ui_quickmessage_streamer", "0");
	...
	{
		self.spectator_blackbg destroy();
...
join_streamer_quicksettings()
{
	...
	self maps\mp\gametypes\global\_global::setClientCvar2("ui_streamer_color", self.pers["streamerSystem_colorMode"]);
...
{
	logprint("_streamer:: onMenuResponse menu=" + menu + ", response=" + response + "\n");
	if (self.pers["team"] != "streamer")
		return;
	logprint("_streamer:: in streamer team\n");

	//if (menu == game["menu_streamersystem"])
	//{
		if (response == "streamersystem_menu_open")//if (response == "menu_open")
		{
		...
		else if (response == "streamersystem_menu_close")//else if (response == "menu_close")
		{
			self.pers["streamerSystem_menu_opened"] = false;
		...
		else if (response == "streamersystem_shift")
		{
			...
			self maps\mp\gametypes\global\_global::setClientCvar2("exec_cmd", "+melee; -melee");
			...
			self.spectator_blackbg = maps\mp\gametypes\global\_global::addHUDClient(self, 0, 0, undefined, undefined, "left", "top", "fullscreen", "fullscreen");
		...
		else if (response == "streamersystem_mouse1" || response == "streamersystem_mouse2")
		{
			...
			if (response == "streamersystem_mouse1") selectedPlayer = level getNextPlayerFrom(level.streamerSystem_player);
			if (response == "streamersystem_mouse2") selectedPlayer = level getPreviousPlayerFrom(level.streamerSystem_player);
		...
		// Middle button
		else if (response == "streamersystem_mouse3")
		{
		...
		// X
		else if (response == "streamersystem_xray")
		{
		...
		// H
		else if (response == "streamersystem_hit")
		{
		...
		// C
		else if (response == "streamersystem_color")
		{
		...
		// Space button
		else if (response == "streamersystem_help")
		{
		...
		// TAB
		else if (response == "streamersystem_score")
		{
		...
		// unexpcted bind
		else if (response == "streamersystem_warning")
		{
			self thread maps\mp\gametypes\_streamer_hud::HUD_CloseMenuWarning_show();
		...
		else
		{
			i = -1;
	    		switch(response)
			{
				case "streamersystem_follow_1": i = 1; break;
				case "streamersystem_follow_2": i = 2; break;
				case "streamersystem_follow_3": i = 3; break;
				case "streamersystem_follow_4": i = 4; break;
				case "streamersystem_follow_5": i = 5; break;
				case "streamersystem_follow_6": i = 6; break;
				case "streamersystem_follow_7": i = 7; break;
				case "streamersystem_follow_8": i = 8; break;
				case "streamersystem_follow_9": i = 9; break;
				case "streamersystem_follow_10": i = 10; break;
			}
...
openStreamerMenu()
{
	...
	while(!self.pers["streamerSystem_menu_open_confirmed"] && self.pers["team"] == "streamer")
	{
		...
		self maps\mp\gametypes\global\_global::setClientCvar2("ui_quickmessage_streamer", "1");
...
*/
		...
		if (self.pers["streamerSystem_menu_opened"])
		{
			...
			{
				self maps\mp\gametypes\global\_global::setClientCvar2("cl_bypassMouseInput", cl_bypassMouseInput);
...
getRecommandedPlayer()
{
	...
	*/
	...
	{
		if (game["round"] == 5)
			selectedPlayer = findPlayerWithWeaponClass("allies", "boltaction"); // original "shotgun"
	...
	{
		if (game["round"] == 6)
			selectedPlayer = findPlayerWithWeaponClass("axis", "boltaction"); // original "shotgun"
...
waitForSpectatorsInKillcam()
{
	...
	if (in_killcam)
	{
		...
		hud maps\mp\gametypes\global\_global::hideHUDSmooth(.5);
		...
		if (maxTime > 15 && ((level.in_readyup && level.playersready) || (!level.in_readyup && level.in_strattime)))
		{
			...
			for(i = 0; i < players.size; i++)
			{
				...
				if (player.pers["team"] == "allies" || player.pers["team"] == "axis")
				{
					...
					name = maps\mp\gametypes\global\_global::removeColorsFromString(player.name, true); // true = remove only double colors
