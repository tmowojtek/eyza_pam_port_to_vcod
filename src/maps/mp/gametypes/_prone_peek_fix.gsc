...
Init()
{
	logprint("_prone_peek_fix::init\n");
	
	maps\mp\gametypes\global\_global::addEventListener("onCvarChanged", ::onCvarChanged);

	maps\mp\gametypes\global\_global::registerCvar("scr_prone_peek_fix", "BOOL", 0);				// level.scr_prone_peek_fix
				...
				if(forwardTrace["fraction"] < 1)
				{
					...
					if (level.debug_pronepeek)
					{
						if (!isDefined(self.pronepeek_waypoint1))
							self.pronepeek_waypoint1 = maps\mp\gametypes\global\_global::newClientHudElem2(self);
					...
					if(forwardTrace["fraction"] < 1)
					{
						...
						if (level.debug_pronepeek)
						{
							if (!isDefined(self.pronepeek_waypoint2))
								self.pronepeek_waypoint2 = maps\mp\gametypes\global\_global::newClientHudElem2(self);
