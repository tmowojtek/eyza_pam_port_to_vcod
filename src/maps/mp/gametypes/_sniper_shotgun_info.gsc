...
init()
{
	logprint("_sniper_shotgun_info::init\n");
...
updateSniperShotgunHUD()
{
	...
	for(;;)
	{
		...
		for(i = 0; i < players.size; i++)
		{
			...
			if(isDefined(player.pers["weapon"]))
			{
				...
				if (player.sessionstate == "playing")
					text = maps\mp\gametypes\global\_global::removeColorsFromString(player.name) + "\n" + color + weapon1 + " / " + weapon2;
				else
					text = maps\mp\gametypes\global\_global::removeColorsFromString(player.name) + "\n" + color + "-";
		...
		for(i = 0; i < players.size; i++)
		{
			...
			if (exit)
			{
				player maps\mp\gametypes\global\_global::setClientCvarIfChanged("ui_sniper_info", "");
				//player maps\mp\gametypes\global\_global::setClientCvarIfChanged("ui_shotgun_info", "");
			}
			else
			{
				if (player.pers["team"] == "allies")
				{
					player maps\mp\gametypes\global\_global::setClientCvarIfChanged("ui_sniper_info", allies_sniper);
					//player maps\mp\gametypes\global\_global::setClientCvarIfChanged("ui_shotgun_info", allies_shotgun);
				}
				else if (player.pers["team"] == "axis")
				{
					player maps\mp\gametypes\global\_global::setClientCvarIfChanged("ui_sniper_info", axis_sniper);
					//player maps\mp\gametypes\global\_global::setClientCvarIfChanged("ui_shotgun_info", axis_shotgun);
				}
				else // leaved team
				{
					player maps\mp\gametypes\global\_global::setClientCvarIfChanged("ui_sniper_info", "");
