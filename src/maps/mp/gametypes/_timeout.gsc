...
init()
{
	logprint("_timeout::init\n");
	
	maps\mp\gametypes\global\_global::addEventListener("onCvarChanged", ::onCvarChanged);

	maps\mp\gametypes\global\_global::registerCvarEx("C", "scr_timeouts", "INT", 0, 0, 10);
	maps\mp\gametypes\global\_global::registerCvarEx("C", "scr_timeouts_half", "INT", 0, 0, 10);
	maps\mp\gametypes\global\_global::registerCvarEx("C", "scr_timeout_length", "FLOAT", 5, 0, 1440);


	if(game["firstInit"])
	{
		maps\mp\gametypes\global\_global::precacheString2("STRING_TIMEOUT", &"Time-out");
		maps\mp\gametypes\global\_global::precacheString2("STRING_GOING_TO_TIMEOUT", &"Going to Time-out");
		maps\mp\gametypes\global\_global::precacheString2("STRING_GOING_TO_TIMEOUT_IN", &"Going to Time-out in");
...
Start_Timeout_Mode(runned_in_middle_of_game)
{
	...
	wait 0.05; // wait untill streamer system is initialized
	...
	if (level.gametype == "sd" || level.gametype == "re")
	{
		...
		logprint("_timeout:: Invoked map_restart(true)\n");
		map_restart(true);
	}
}
...
Update_Player_HUD_Cvar()
{
	...
	self maps\mp\gametypes\global\_global::setClientCvarIfChanged("ui_allow_timeout", number);
	...
	if (number == 1 && (self.pers["team"] == "axis" || self.pers["team"] == "allies"))
	{
		...
		self maps\mp\gametypes\global\_global::setClientCvarIfChanged("ui_timeout_info", str);
	}
	else
		self maps\mp\gametypes\global\_global::setClientCvarIfChanged("ui_timeout_info", "");
}
...
HUD_Timeout()
{
	...
	// Time-out
	timeout = maps\mp\gametypes\global\_global::addHUD(x, y, 1.2, (1,1,0), "center", "middle", "right");
	timeout maps\mp\gametypes\global\_global::showHUDSmooth(.5);
	timeout setText(game["STRING_TIMEOUT"]);
	y += 20;

	// Going to Time-out
	going_timeout = maps\mp\gametypes\global\_global::addHUD(x, y, 1.2, (1,1,0), "center", "middle", "right");
	going_timeout maps\mp\gametypes\global\_global::showHUDSmooth(.5);
...
TO_Time_Based_GoingToTimeOutIN()
{
	...
	goingtoo = maps\mp\gametypes\global\_global::newHudElem2();
	goingtoo.x = 320;
	goingtoo.y = 75; //220
	...
	ttb = maps\mp\gametypes\global\_global::newHudElem2();
	ttb.x = 320;
	ttb.y = 90; //220
	ttb.alignX = "center";
	ttb.alignY = "top";
